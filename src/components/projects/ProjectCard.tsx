...
export function ProjectCard({ project, industryName }: { project: Project; industryName?: string }) {
  return (
    <Link
      ...
      className="group relative block overflow-hidden rounded-[4px] aspect-[4/3]"
    >
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col items-center p-6 text-center">
        ...
        <span className="mt-4 inline-flex translate-y-2 items-center gap-1.5 rounded-[2px] border border-white/35 bg-white/10 px-4 py-1.5 text-xs font-medium text-(--color-paper) opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          View project
